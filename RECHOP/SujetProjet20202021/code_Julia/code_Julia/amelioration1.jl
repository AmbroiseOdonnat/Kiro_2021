...
sol_flows = open("solution/flows_europe.txt") do file
    readlines(file)
end

routes_flows = [lire_route(sol_flows[1+r]) for r=1:length(sol_flows)-1]

instance_flow = lire_solution(instance_zone,"solution/flows_europe.txt")
...
write_solution_to_file(good_instance, "solution/routes_binpack.txt")
