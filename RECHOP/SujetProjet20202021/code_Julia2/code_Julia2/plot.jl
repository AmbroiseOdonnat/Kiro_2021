...
function ajouter_route_plot!(route::Route, instance::Instance, pl::Plots.Plot)
...
function plot_routes(instance::Instance; j::Int)::Plots.Plot
    ...
    for route in instance.routes
        if route.j == j
            ajouter_route_plot!(route, instance, pl)
        end
    end
...
function plot_stocks(fournisseur::Fournisseur)::Plots.Plot
    E, J = size(fournisseur.s)
    pl = plot()
    for e = 1:E
        plot!(pl, 1:J, fournisseur.s[e, :], label = "e=$e", lw = 3, markershape = :circle)
    end
    plot!(
        pl,
...
function geojson_routes(instance::Instance)
    usines, fournisseurs = instance.usines, instance.fournisseurs
...
function plot_folium(
    instance::Instance;
    include_fournisseurs::Bool = false,
    ...
    all_routes_map = folium.FeatureGroup(name="toutes les routes", show=$include_routes)
    routes_map = [folium.FeatureGroup(name="routes du jour {}".format(j), show=False) for j in range($instance.J)]
    """
    ...
    for route in instance.routes
        ...
        folium.PolyLine(
            ...
            color="black",
            opacity=0.3,
            popup="Route - jour {}, usine {}".format($j, $u)
        ).add_to(routes_map[$j])
        folium.PolyLine(
            locations=$path,
            color="black",
    ...
    all_routes_map.add_to(m)
    for j in range($instance.J):
        routes_map[j].add_to(m)
