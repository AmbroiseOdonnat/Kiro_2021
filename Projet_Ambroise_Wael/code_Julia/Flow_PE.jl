function graph_b_flow(usine::Usine,fournisseur::Fournisseur,emballage::Emballage,instance::Instance)::Data
    ...
    vertex_nb = 1 + 1 + 2*(instance.J-1) + 1 + 4*(instance.J-1)
    ...
    for i in 2:(2*instance.J)
        if i%2==1
            ...
            data_test.upper_cap[(i,i+1)] = typemax(Int)
            ...
            if i<42
                ...
                data_test.lower_cap[(i,i+1)] = usine.r[emballage.e,jour]
                data_test.upper_cap[(i,i+1)] = typemax(Int)
                ...
                data_test.upper_cap[(i, 2*(instance.J-1) + jour*4 + 3)] = typemax(Int)
            else
                # arc avec carton
                add_edge!(data_test.graph,i,1,0)
                data_test.lower_cap[(i,1)] = 0
                data_test.upper_cap[(i,1)] = typemax(Int)
            end
        end
    end
    jour = 1
    for i in (2*instance.J+1):vertex_nb
        if i%4==3
            ...
            if i<vertex_nb
                ...
                data_test.upper_cap[(i,i+3)] = typemax(Int)
                # arc avec le jour d'après
    ...
    for i in (2*instance.J+1):vertex_nb
        if i%4==1
            ...
            data_test.upper_cap[(1,i)] = typemax(Int)
        end
    end
    return data_test
end
