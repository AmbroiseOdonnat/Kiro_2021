# première solution après Bin Packing
include("/Users/ambroise/Projet/code_Julia/import_all.jl")
using LinearAlgebra
#recuperation routes
zone = "europe"
sol_flows = open("/Users/ambroise/Projet/sujet/routes_binpack_$zone.txt") do file
    readlines(file)
end
...
instance_flow = lire_solution(instance_zone,"/Users/ambroise/Projet/sujet/routes_binpack_$zone.txt")
...
#test
test_instance_flow = lire_solution(instance_zone,"/Users/ambroise/Projet/sujet/routes_binpack_$zone.txt")
...
write_solution_to_file(test_instance_flow,"/Users/ambroise/Projet/sujet/solution_europe_route_4.txt")
#faisabilité
...
function same_uj(routes::Vector{Route},instance::Instance)
    same_uj = Array{Vector{Route}}(undef,instance.U,instance.J)
    for u in 1:instance.U, j in 1:instance.J
        same_uj[u,j] = Route[]
    end
    for route in routes
        push!(same_uj[route.u, route.j], route)
    end
    return same_uj
end

function sorted_routes(routes::Vector{Route}, instance::Instance)
    Chargement = sort!([(Q(routes[i],instance),i) for i in 1:length(routes)],rev=true)
    return [routes[q[2]] for q in Chargement]
end

sorted_routes(routes_flows,instance_flow)
...
function optimiser_routes(routes::Vector{Route},instance::Instance)::Vector{Route}
    ...
    L = instance.L
    routes_final = []
    same_uj_Routes = same_uj(routes,instance)
    for r in same_uj_Routes
        global mut_routes_uj = sorted_routes(r,instance)
...
##espace de test
test_route = test_instance.routes
routes_uj(test_instance,test_route,test_instance.usines[1],1)
##espace de test
name = "europe"
bis_test_instance = lire_instance("/Users/ambroise/Projet/sujet/$name.csv")
bis_sol_flows = open("/Users/ambroise/Projet/sujet/routes_binpack_$name.txt") do file
    readlines(file)
end

bis_routes_flows = [lire_route(bis_sol_flows[1+r]) for r=1:length(bis_sol_flows)-1]
#opti_bis_routes_flows = optimiser_routes(bis_routes_flows,bis_test_instance)
##test sur europe après le Bin Packing

bis_instance_zone = lire_instance("/Users/ambroise/Projet/sujet/$name.csv")
bis_instance_flow = lire_solution(bis_instance_zone,"/Users/ambroise/Projet/sujet/routes_binpack_$name.txt")
opti_bis_routes_flows = optimiser_routes(bis_routes_flows,bis_instance_zone)
bis_instance_flow.routes = opti_bis_routes_flows
bis_instance_flow.R = length(opti_bis_routes_flows)
update_stocks!(bis_instance_flow,opti_bis_routes_flows)
#faisabilité
bis_instance_flow
feasibility(bis_instance_flow)
#cout
cost_verbose(bis_instance_flow)

write_solution_to_file(bis_instance_flow,"/Users/ambroise/Projet/sujet/solution_europe_best_route_4.txt") #meilleure solution trouvée
##test sur le flot sortant du PLNE
name = "europe"
bis_test_instance = lire_instance("/Users/ambroise/Projet/sujet/$name.csv")
bis_sol_flows = open("/Users/ambroise/Projet/sujet/flows_$name.txt") do file
    readlines(file)
end

bis_routes_flows = [lire_route(bis_sol_flows[1+r]) for r=1:length(bis_sol_flows)-1]
bis_instance_zone = lire_instance("/Users/ambroise/Projet/sujet/$name.csv")
bis_instance_flow = lire_solution(bis_instance_zone,"/Users/ambroise/Projet/sujet/flows_$name.txt")
opti_bis_routes_flows = optimiser_routes(bis_routes_flows,bis_instance_zone)
bis_instance_flow.routes = opti_bis_routes_flows
bis_instance_flow.R = length(opti_bis_routes_flows)
update_stocks!(bis_instance_flow,opti_bis_routes_flows)
